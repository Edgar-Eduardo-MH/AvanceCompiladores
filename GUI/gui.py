...
class Compiler_GUI:
    def __init__(self, root):
        ...
        # Scrollbar vertical
        scrollbar = tk.Scrollbar(text_frame, command=self.sync_scroll, bg=self.BORDER_COLOR)
        scrollbar.grid(row=0, column=2, sticky="ns")
        self.code_text_area.config(yscrollcommand=scrollbar.set)
        self.line_numbers.config(yscrollcommand=scrollbar.set)

        # Scrollbar horizontal
        h_scrollbar = tk.Scrollbar(text_frame, orient=tk.HORIZONTAL, command=self.code_text_area.xview, bg=self.BORDER_COLOR)
        ...
        self.line_column_label = tk.Label(root, text="Línea: 1, Columna: 1", bd=1, relief=tk.SUNKEN, anchor=tk.W, bg=self.STATUS_BAR_BG, fg=self.TEXT_COLOR)
    ...
    def configure_styles(self):
        ...
        style.configure("TNotebook",background=self.FRAME_BG, borderwidth=0)    
        style.configure("TNotebook.Tab", background=self.LINE_NUM_BG, foreground=self.TEXT_COLOR, padding=(10,5), font=("Arial", 12, "bold"))
        style.map("TNotebook.Tab", background=[("selected","#101018")])
        ...
        style.configure("TButton", background=self.BUTTON_BG, foreground=self.BUTTON_FG, font=("Arial", 10, "bold"), padding=(5,5))
        style.map("TButton", background=[("active","#4850D4")])
        style.configure("TEntry", background=self.ENTRY_BG, foreground=self.TEXT_COLOR, insertcolor=self.TEXT_COLOR, padding=(5,5)) 
